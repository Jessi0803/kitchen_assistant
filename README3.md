...
| **模型推理** | PyTorch 引擎 | CoreML 引擎（自動選擇硬體） | Server 端使用 PyTorch 的 C++ 後端進行張量運算，支援完整的算子集合。iOS 端使用 Apple 的 CoreML 引擎，會根據模型和裝置自動選擇最佳硬體：優先使用 Neural Engine (ANE)，其次是 Metal GPU，最後才是 CPU。不同硬體的運算子實現有細微差異（如浮點數精度處理、矩陣乘法算法），會累積成微小誤差。 | **Server 端：** PyTorch C++ 後端<br>**iOS 端：** CoreML 自動選擇<br>• M 系列/A 系列：Neural Engine (ANE)<br>• 較舊裝置：Metal GPU 或 CPU |
